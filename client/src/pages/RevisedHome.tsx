...
function ToggleGroup({ group, childNames, ratings, onScore, onRemark, onCompleteGroup, defaultOpen }: { group: Section["contexts"][number]["groups"][number]; childNames: string[]; ratings: RatingData; onScore: (itemId: string, childIndex: number, score: Rating) => void; onRemark: (itemId: string, childIndex: number, remark: string) => void; onCompleteGroup: (items: Section["contexts"][number]["groups"][number]["items"], childIndex: number) => void; defaultOpen: boolean }) {
  ...
  return <article className="overflow-hidden rounded-xl border border-[#E5E7F0] bg-white">
    <button type="button" onClick={() => setOpen((current) => !current)} aria-expanded={open} className="flex w-full items-center justify-between gap-3 border-l-4 border-[#8E80F6] bg-[#F8F7FF] px-3 py-3 text-left transition hover:bg-[#F3F1FF]"><span className="whitespace-nowrap text-[15px] font-bold text-[#3B456B]">{cleanHeading(group.title)}</span><ChevronDown className={`h-4 w-4 shrink-0 text-[#7466E0] transition ${open ? "rotate-180" : ""}`} /></button>
    {open && <div className="overflow-x-auto"><div className="min-w-[650px]"><div className="grid border-y border-[#E9EBF3] bg-[#EEF0FA]" style={{ gridTemplateColumns: columns }}><div className="px-3 py-2 text-[11px] font-bold text-[#59638E]">評分項目</div>{childNames.map((name, index) => <button key={index} type="button" onClick={() => onCompleteGroup(group.items, index)} className={`border-l border-[#E2E4F0] px-2 py-2 text-center text-[11px] font-bold text-[#58658F] transition hover:bg-[#E7F8F0] hover:text-[#16835E] active:scale-[0.98] ${childColumnClass(index)}`} title={`將 ${displayChildName(name, index)} 在此小範疇的所有評分標示為 2`}>{displayChildName(name, index)}<span className="mt-0.5 block text-[9px] font-medium text-[#8B94B6]">按此全部設為 2</span></button>)}</div>{group.items.map((item, itemIndex) => <div key={item.id} className="grid border-b border-[#EEF0F6] last:border-b-0" style={{ gridTemplateColumns: columns }}><div className="px-3 py-3"><div className="flex gap-2"><span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#F0EFFF] text-[10px] font-bold text-[#6557DB]">{itemIndex + 1}</span><p className="text-[15px] leading-6 font-semibold text-[#323A61] sm:text-[16px]"><ItemText text={item.text} /></p></div></div>{childNames.map((_, childIndex) => { const entry = ratings[entryKey(item.id, childIndex)]; return <div key={childIndex} className={`border-l border-[#EEF0F6] p-2 ${childColumnClass(childIndex)}`}><RatingButtons score={entry?.score} onChange={(score) => onScore(item.id, childIndex, score)} /><textarea value={entry?.remark ?? ""} onChange={(event) => onRemark(item.id, childIndex, event.target.value)} aria-label={`${displayChildName(childNames[childIndex], childIndex)} 對「${item.text.split("\n")[0]}」的備註`} placeholder="備註" rows={1} className="mt-1 block h-8 w-full resize-y rounded-md border border-[#E3E5EE] bg-white/85 px-1.5 py-1 text-[10px] leading-3.5 text-[#596188] outline-none placeholder:text-[#B3B7CD] focus:border-[#8275FF] focus:bg-white" /></div>; })}</div>)}</div></div>}
  </article>;
}
...
function ToggleContext({ context, childNames, ratings, onScore, onRemark, onCompleteGroup, defaultOpen }: { context: Section["contexts"][number]; childNames: string[]; ratings: RatingData; onScore: (itemId: string, childIndex: number, score: Rating) => void; onRemark: (itemId: string, childIndex: number, remark: string) => void; onCompleteGroup: (items: Section["contexts"][number]["groups"][number]["items"], childIndex: number) => void; defaultOpen: boolean }) {
  const [open, setOpen] = useState(defaultOpen);
  return <section className="mt-3"><button type="button" onClick={() => setOpen((current) => !current)} aria-expanded={open} className="flex w-full items-center justify-between rounded-xl border border-[#CFC8FF] bg-[linear-gradient(90deg,#EDEAFF,#F7F5FF)] px-4 py-3.5 text-left shadow-[0_3px_10px_rgba(104,87,214,0.06)]"><span className="whitespace-nowrap text-[16px] font-extrabold text-[#4F46A4]">{cleanHeading(context.title)}</span><ChevronDown className={`h-4 w-4 text-[#6558D6] transition ${open ? "rotate-180" : ""}`} /></button>{open && <div className="mt-2 space-y-2">{context.groups.map((group, index) => <ToggleGroup key={`${context.title}-${group.title}`} group={group} childNames={childNames} ratings={ratings} onScore={onScore} onRemark={onRemark} onCompleteGroup={onCompleteGroup} defaultOpen={defaultOpen && index === 0} />)}</div>}</section>;
}
...
export default function RevisedHome() {
  ...
  const exportExcel = () => {
    ...
    Array.from(new Set(areas.map((area) => area.category))).forEach((category) => {
      ...
      const rows = [["大類別", "小範疇", ...childNames.map(displayChildName)], ...categoryAreas.map((area) => [cleanHeading(area.context), cleanHeading(area.group), ...area.scores.map((score) => score.average === undefined ? "" : Number(score.average.toFixed(1)))])];
