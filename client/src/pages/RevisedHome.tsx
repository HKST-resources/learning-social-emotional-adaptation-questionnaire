...
function ToggleGroup({ group, childNames, ratings, onScore, onRemark, defaultOpen }: { group: Section["contexts"][number]["groups"][number]; childNames: string[]; ratings: RatingData; onScore: (itemId: string, childIndex: number, score: Rating) => void; onRemark: (itemId: string, childIndex: number, remark: string) => void; defaultOpen: boolean }) {
  ...
  return <article className="overflow-hidden rounded-xl border border-[#E5E7F0] bg-white">
    <button type="button" onClick={() => setOpen((current) => !current)} aria-expanded={open} className="flex w-full items-center justify-between gap-3 bg-[#FBFBFF] px-3 py-3 text-left transition hover:bg-[#F7F6FF]"><span className="whitespace-nowrap text-[15px] font-bold text-[#3B456B]">{cleanHeading(group.title)}</span><ChevronDown className={`h-4 w-4 shrink-0 text-[#7466E0] transition ${open ? "rotate-180" : ""}`} /></button>
...
function ToggleContext({ context, childNames, ratings, onScore, onRemark, defaultOpen }: { context: Section["contexts"][number]; childNames: string[]; ratings: RatingData; onScore: (itemId: string, childIndex: number, score: Rating) => void; onRemark: (itemId: string, childIndex: number, remark: string) => void; defaultOpen: boolean }) {
  const [open, setOpen] = useState(defaultOpen);
  return <section className="mt-3"><button type="button" onClick={() => setOpen((current) => !current)} aria-expanded={open} className="flex w-full items-center justify-between rounded-xl border border-[#DCD8FF] bg-[linear-gradient(90deg,#F4F2FF,#FBF8FF)] px-3 py-3 text-left"><span className="whitespace-nowrap text-[16px] font-extrabold text-[#544BB6]">{cleanHeading(context.title)}</span><ChevronDown className={`h-4 w-4 text-[#6558D6] transition ${open ? "rotate-180" : ""}`} /></button>{open && <div className="mt-2 space-y-2">{context.groups.map((group, index) => <ToggleGroup key={`${context.title}-${group.title}`} group={group} childNames={childNames} ratings={ratings} onScore={onScore} onRemark={onRemark} defaultOpen={defaultOpen && index === 0} />)}</div>}</section>;
}
...
function CategoryPanel({ section, sectionIndex, childNames, ratings, onScore, onRemark }: { section: Section; sectionIndex: number; childNames: string[]; ratings: RatingData; onScore: (itemId: string, childIndex: number, score: Rating) => void; onRemark: (itemId: string, childIndex: number, remark: string) => void }) {
  ...
  return <section id={`domain-${sectionIndex}`} className="scroll-mt-28 overflow-hidden rounded-2xl border border-[#E4E6F1] bg-white shadow-[0_6px_20px_rgba(53,56,106,0.06)]"><button type="button" onClick={() => setOpen((current) => !current)} aria-expanded={open} className="flex w-full items-center justify-between gap-3 bg-[linear-gradient(100deg,#F0EEFF,#FAF8FF)] px-4 py-4 text-left"><h2 className="whitespace-nowrap text-[20px] font-bold text-[#40388F]">{cleanHeading(section.title)}</h2><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-[#6759DA] shadow-sm"><ChevronDown className={`h-4 w-4 transition ${open ? "rotate-180" : ""}`} /></span></button>{open && <div className="p-3 sm:p-4">{section.contexts.map((context) => <ToggleContext key={`${section.title}-${context.title}`} context={context} childNames={childNames} ratings={ratings} onScore={onScore} onRemark={onRemark} defaultOpen={false} />)}</div>}</section>;
}
...
export default function RevisedHome() {
  ...
  const exportExcel = () => {
    ...
    Array.from(new Set(areas.map((area) => area.category))).forEach((category) => {
      ...
      const rows = [["大類別", "小範疇", ...childNames.map(displayChildName)], ...categoryAreas.map((area) => [cleanHeading(area.context), cleanHeading(area.group), ...area.scores.map((score) => score.needPercentage === undefined ? "" : `${Math.round(score.needPercentage)}%`)])];
...
function PrintableAnalysis({ ratingDate, childNames, ratings, selectedGoals }: { ratingDate: string; childNames: string[]; ratings: RatingData; selectedGoals: string[] }) {
  ...
  return <article className="print-report hidden"><header><p>學習、社交及情緒適應問卷</p><h1>各兒童表現分析及已選訓練目標</h1><div className="print-meta"><span>評估日期：{ratingDate}</span><span>兒童：{childNames.map(displayChildName).join("、")}</span></div></header>{categories.map((category) => <section key={category} className="print-section"><h2>{category}</h2><table><thead><tr><th>大類別</th><th>小範疇</th>{childNames.map((name, index) => <th key={index}>{displayChildName(name, index)}</th>)}</tr></thead><tbody>{areas.filter((area) => area.category === category).map((area) => <tr key={area.key}><td>{cleanHeading(area.context)}</td><td>{cleanHeading(area.group)}</td>{area.scores.map((score, index) => <td key={index}>{score.needPercentage === undefined ? "" : `${Math.round(score.needPercentage)}%`}</td>)}</tr>)}</tbody></table></section>)}<section className="print-section"><h2>已選訓練目標</h2>{selectedGoalItems.length ? <table><thead><tr><th>大類別</th><th>小範疇／項目</th>{childNames.map((name, index) => <th key={index}>{displayChildName(name, index)}</th>)}</tr></thead><tbody>{selectedGoalItems.map((item) => <tr key={item.id}><td>{cleanHeading(item.context)}</td><td><ItemText text={item.text} /></td>{childNames.map((_, childIndex) => { const key = goalKey(item.id, childIndex); return <td key={childIndex}>{selectedGoals.includes(key) ? `✓ ${ratings[key]?.score ?? ""}` : ""}</td>; })}</tr>)}</tbody></table> : <p>尚未選擇個別訓練目標。</p>}</section></article>;
}
