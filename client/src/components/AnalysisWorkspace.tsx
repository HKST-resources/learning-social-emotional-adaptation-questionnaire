...
function printableOverview(category: string, areas: SummaryArea[], childNames: string[], ratings: AnalysisRatingData, expandedKeys: Set<string>, scope: PrintScope) {
  ...
  const rows = areas.map((area) => {
    ...
    const details = includeDetails ? area.items.map((item, index) => `<tr class="detail-row"><td></td><td><span class="item-number">${index + 1}</span>${escapeReportHtml(item.text)}</td>${childNames.map((_, childIndex) => scoreCell(ratings[goalKey(item.id, childIndex)]?.score)).join("")}</tr>`).join("") : "";
  ...
  return `<section class="report-section"><h2>${escapeReportHtml(category)}</h2><p>格內數字為小範疇平均分數。${includeDetails ? "此完整報告已包括所有小範疇的詳細評分。" : ""}</p><table><thead><tr><th>類別</th><th>小範疇</th>${childNames.map((name, index) => `<th>${escapeReportHtml(displayChildName(name, index))}</th>`).join("")}</tr></thead><tbody>${rows}</tbody></table></section>`;
}
...
function printableGoals(selectedGoalItems: AnalysisItem[], childNames: string[], selectedGoals: string[]) {
  ...
  const childColumnWidth = `${26 / Math.max(childNames.length, 1)}%`;
  const rows = selectedGoalItems.map((item) => `<tr><td style="width:8%">${escapeReportHtml(cleanHeading(item.context))}</td><td style="width:12%">${escapeReportHtml(cleanHeading(item.group))}</td><td style="width:54%">${escapeReportHtml(item.text)}</td>${childNames.map((_, childIndex) => `<td class="goal-mark" style="width:${childColumnWidth}">${selectedGoals.includes(goalKey(item.id, childIndex)) ? "✓" : ""}</td>`).join("")}</tr>`).join("");
  return `<section class="report-section goals-section"><h2>已選訓練目標</h2><table class="goals-table"><thead><tr><th style="width:8%">類別</th><th style="width:12%">小範疇</th><th style="width:54%">訓練目標</th>${childNames.map((name, index) => `<th style="width:${childColumnWidth}">${escapeReportHtml(displayChildName(name, index))}</th>`).join("")}</tr></thead><tbody>${rows}</tbody></table></section>`;
}
...
function buildPrintableReport({ scope, ratingDate, childNames, areas, ratings, expandedKeys, selectedGoalItems, selectedGoals }: { scope: PrintScope; ratingDate: string; childNames: string[]; areas: SummaryArea[]; ratings: AnalysisRatingData; expandedKeys: Set<string>; selectedGoalItems: AnalysisItem[]; selectedGoals: string[] }) {
  ...
  return `<!doctype html><html lang="zh-HK"><head><meta charset="utf-8" /><title>${escapeReportHtml(printScopeMeta[scope].printTitle)}</title><style>@page{size:A4 landscape;margin:8mm}*{box-sizing:border-box}body{margin:0;color:#252A48;font-family:"Noto Sans TC","PingFang TC",sans-serif;font-size:9pt}header{border-bottom:2px solid #765EEF;margin-bottom:12px;padding-bottom:8px}h1{margin:0;color:#323A68;font-family:"Noto Serif TC",serif;font-size:20pt}header p{margin:4px 0 0;color:#69729A}.report-section{break-inside:auto;page-break-inside:auto;margin:16px 0}.report-section h2{break-after:avoid;page-break-after:avoid;margin:0 0 5px;padding-left:8px;border-left:4px solid #765EEF;color:#394274;font-size:14pt}.report-section p{margin:0 0 7px;color:#6E7698;font-size:8pt}table{width:100%;border-collapse:collapse;table-layout:fixed;break-inside:auto}thead{display:table-header-group}tr{break-inside:avoid;page-break-inside:avoid}th,td{border:1px solid #CAD0DF;padding:4px;vertical-align:top;overflow-wrap:anywhere;word-break:break-word;line-height:1.38}th{background:#F1F0FF;color:#465081;font-weight:700;text-align:left}th:first-child,td:first-child{width:11%}th:nth-child(2),td:nth-child(2){width:30%}td.mean,.score,.goal-mark{text-align:center;vertical-align:middle;font-weight:800}.summary-row:nth-child(4n+1) td{background:#FBFAFF}.detail-row td{background:#F5F3FF;color:#433D83}.detail-row td:first-child{background:#EFEAFF}.item-number{display:inline-flex;width:15px;height:15px;align-items:center;justify-content:center;margin-right:4px;border-radius:50%;background:#7465E9;color:#fff;font-size:7pt;font-weight:700}.score{border-radius:0}.score-0{background:#FFF0F2;color:#BA3A50}.score-1{background:#FFF7D9;color:#9A6800}.score-2{background:#EDFCF4;color:#137D59}.empty{color:#AAB1C6}.goal-mark{color:#13945F;font-size:13pt}.goals-section{break-before:page;page-break-before:always}.scope-goals .goals-section{break-before:auto;page-break-before:auto;margin-top:10px}.goals-section h2{color:#217257}.goals-table th:first-child,.goals-table td:first-child{width:8%}.goals-table th:nth-child(2),.goals-table td:nth-child(2){width:12%}.goals-table th:nth-child(3),.goals-table td:nth-child(3){width:54%}.scope-overview header{margin-bottom:4px;padding-bottom:4px}.scope-overview h1{font-size:15pt}.scope-overview header p{margin-top:2px;font-size:7pt}.scope-overview .report-section:not(.goals-section){margin:5px 0}.scope-overview .report-section:not(.goals-section) h2{margin-bottom:2px;padding-left:5px;border-left-width:3px;font-size:10pt}.scope-overview .report-section:not(.goals-section) p{display:none}.scope-overview .report-section:not(.goals-section) table{font-size:7pt}.scope-overview .report-section:not(.goals-section) th,.scope-overview .report-section:not(.goals-section) td{padding:2px 3px;line-height:1.12}.scope-overview .report-section:not(.goals-section) th:first-child,.scope-overview .report-section:not(.goals-section) td:first-child{width:10%}.scope-overview .report-section:not(.goals-section) th:nth-child(2),.scope-overview .report-section:not(.goals-section) td:nth-child(2){width:31%}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body class="scope-${scope}"><header><h1>${escapeReportHtml(printScopeMeta[scope].printTitle)}</h1><p>評估日期：${escapeReportHtml(ratingDate)}　｜　${childNames.length} 位兒童</p></header>${overview}${goals}</body></html>`;
}
