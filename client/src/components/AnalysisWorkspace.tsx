 ...
 * Analysis is a clear, high-contrast comparison ledger. Gradient emphasis is reserved for
 * areas with the highest 0/1 need percentage, while detailed ratings remain evidence-led.
 */
...
function needGradient(rank: number | undefined, percentage: number | undefined) {
  if (percentage === undefined) return { className: "border-[#E5E7EF] bg-white text-[#B0B5C8] hover:border-[#B9BFE0]", style: undefined };
  if (rank !== undefined) {
    const gradients = [
      "linear-gradient(135deg,#ED3E78 0%,#FF6F69 48%,#FFB350 100%)",
      "linear-gradient(135deg,#F05791 0%,#FF7D89 56%,#FFBD62 100%)",
      "linear-gradient(135deg,#E86CB3 0%,#FF93A0 58%,#FFC56C 100%)",
      "linear-gradient(135deg,#A767E9 0%,#F18ABB 57%,#FFCC75 100%)",
      "linear-gradient(135deg,#7D79E9 0%,#B58FEA 56%,#FFD07A 100%)",
    ];
    return { className: "border-transparent text-white shadow-[0_5px_12px_rgba(217,76,118,0.23)]", style: { backgroundImage: gradients[rank] } };
  }
  if (percentage >= 75) return { className: "border-[#FFC9B5] bg-[#FFF0EB] text-[#B8503E]", style: undefined };
  if (percentage >= 50) return { className: "border-[#FFE1A7] bg-[#FFF8E8] text-[#9A6B00]", style: undefined };
  if (percentage > 0) return { className: "border-[#D9D7FF] bg-[#F6F5FF] text-[#6259BE]", style: undefined };
  return { className: "border-[#CDEBDD] bg-[#F4FFF9] text-[#21815E]", style: undefined };
}
...
function OverviewTable({ category, areas, childNames, ratings, scoreFilter, selectedGoals, expandedCells, onExpand, onToggleGoal }: {
  ...
  const topNeedAreas = useMemo(() => childNames.map((_, childIndex) => new Map(
    areas
      .filter((area) => (area.scores[childIndex].needPercentage ?? 0) > 0)
      .sort((left, right) => (right.scores[childIndex].needPercentage ?? 0) - (left.scores[childIndex].needPercentage ?? 0) || (right.scores[childIndex].needCount - left.scores[childIndex].needCount))
      .slice(0, 5)
  ...
  return <section className="mt-7">
    <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
      ...
      <p className="text-[12px] text-[#757FA6]">格內數字為此小範疇中 0 分及 1 分項目的百分比；亮色漸變為每位兒童最高的 5 個百分比。</p>
    </div>
    <div className="overflow-x-auto rounded-2xl border border-[#E1E4EF] bg-white shadow-[0_8px_24px_rgba(42,45,88,0.06)]">
      <table className="w-full min-w-[920px] table-fixed border-collapse text-left">
        <thead className="bg-[#F7F8FE]"><tr><th className="w-[160px] border-b border-r border-[#E1E4EF] px-3 py-3 text-[13px] font-bold text-[#57618B]">大類別</th><th className="w-[260px] border-b border-r border-[#E1E4EF] px-3 py-3 text-[13px] font-bold text-[#57618B]">小範疇</th>{childNames.map((name, index) => <th key={index} className="border-b border-r border-[#E1E4EF] px-3 py-3 text-center last:border-r-0"><p className="text-[11px] font-bold text-[#8A91B0]">兒童 {index + 1}</p><p className="mt-0.5 truncate text-[14px] font-extrabold text-[#444F7A]">{displayChildName(name, index)}</p></th>)}</tr></thead>
        <tbody>{areas.map((area) => {
          ...
          return <Fragment key={area.key}>
            <tr className="border-b border-[#EEF0F6] align-stretch"><td className="border-r border-[#E7E9F1] px-3 py-3 text-[13px] font-bold text-[#59638E]">{cleanHeading(area.context)}</td><th scope="row" className="border-r border-[#E7E9F1] px-3 py-3 text-left text-[14px] font-bold leading-5 text-[#30395F]">{cleanHeading(area.group)}</th>{childNames.map((_, childIndex) => {
              const score = area.scores[childIndex]; const cellKey = `${area.key}::${childIndex}`; const tone = needGradient(topNeedAreas[childIndex].get(area.key), score.needPercentage); const selected = expandedCells.has(cellKey);
              return <td key={childIndex} className="border-r border-[#E7E9F1] p-2 text-center last:border-r-0"><button type="button" onClick={() => onExpand(cellKey)} aria-expanded={selected} className={`relative flex min-h-11 w-full flex-col items-center justify-center rounded-lg border px-1 text-[15px] font-extrabold transition active:scale-[0.98] ${tone.className}`} style={tone.style}>{score.needPercentage === undefined ? <span>—</span> : <><span>{Math.round(score.needPercentage)}%</span><span className="mt-0.5 text-[9px] font-semibold opacity-85">0+1</span></>}{selected && <span className="absolute right-1 top-1 h-1.5 w-1.5 rounded-full bg-white/90" />}</button></td>;
            })}</tr>
            {activeChildren.length > 0 && <tr className="border-b border-[#DCE0EC] bg-[#FBFCFF]"><td colSpan={2} className="border-r border-[#E7E9F1] px-3 py-3 align-top"><p className="text-[12px] font-bold text-[#505B87]">{cleanHeading(area.context)} · {cleanHeading(area.group)}</p><p className="mt-1 text-[11px] leading-5 text-[#7B84A7]">已展開 {activeChildren.length} 位兒童的細項；再次按同一格即可收起。</p></td>{childNames.map((name, childIndex) => <td key={childIndex} className="border-r border-[#E7E9F1] px-2 py-2 align-top last:border-r-0">{expandedCells.has(`${area.key}::${childIndex}`) ? <div className="space-y-2">{area.items.filter((item) => { const score = ratings[goalKey(item.id, childIndex)]?.score; return score !== undefined && scoreFilter.has(score); }).map((item) => { const entry = ratings[goalKey(item.id, childIndex)]; const checked = selectedGoals.includes(goalKey(item.id, childIndex)); return <div key={item.id} className="rounded-lg border border-[#E2E5EF] bg-white p-2"><div className="flex gap-2"><span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-md border text-[12px] font-extrabold ${entry?.score === undefined ? "border-[#E5E7EF] text-[#ADB3C6]" : quietRatingClass[entry.score]}`}>{entry?.score ?? "—"}</span><p className="min-w-0 flex-1 text-[12px] leading-5 font-semibold text-[#394264]"><ItemText text={item.text} /></p><button type="button" onClick={() => onToggleGoal(item.id, childIndex)} aria-pressed={checked} aria-label={`選擇 ${displayChildName(name, childIndex)} 的訓練目標：${item.text.split("\n")[0]}`} className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-md border transition ${checked ? "border-[#6557DB] bg-[#6D5DFB] text-white" : "border-[#CBD0E1] bg-white text-transparent hover:border-[#6D5DFB]"}`}><Check className="h-3.5 w-3.5" /></button></div>{entry?.remark ? <p className="mt-2 border-t border-[#EEF0F6] pt-2 text-[11px] leading-4 text-[#737CA1]">備註：{entry.remark}</p> : null}</div>; })}{!area.items.some((item) => { const score = ratings[goalKey(item.id, childIndex)]?.score; return score !== undefined && scoreFilter.has(score); }) && <p className="rounded-lg border border-dashed border-[#D9DDEA] px-2 py-3 text-center text-[11px] text-[#949BB7]">沒有符合篩選的評分。</p>}</div> : null}</td>)}</tr>}
          </Fragment>;
        })}</tbody>
      </table>
    </div>
...
export default function AnalysisWorkspace({ ratingDate, childNames, items, ratings, selectedGoals, onToggleGoal, onBack, onExportExcel }: AnalysisWorkspaceProps) {
  ...
  return <div className="min-h-screen bg-white text-[#20264A]">
    ...
    <main className="mx-auto max-w-[1700px] px-4 pb-10 pt-6 sm:px-7 sm:pt-8"><section className="relative overflow-hidden rounded-[24px] bg-[linear-gradient(120deg,#6D5DFB_0%,#9978FF_44%,#F96EA5_100%)] px-5 py-6 text-white shadow-[0_18px_44px_rgba(109,93,251,0.23)] sm:px-7 sm:py-7"><div className="pointer-events-none absolute -right-8 -top-16 h-48 w-48 rounded-full border-[22px] border-white/20" /><div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><h1 className="text-[30px] font-bold tracking-tight sm:text-[38px]">各兒童表現分析</h1><p className="mt-2 max-w-2xl text-[15px] leading-6 text-white/88">按小範疇比較 0 分及 1 分項目的百分比；可同時展開多個格子，逐項選擇訓練目標。</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => window.print()} className="inline-flex h-10 items-center gap-2 rounded-xl bg-white px-4 text-[13px] font-bold text-[#5B50D8] transition hover:bg-[#F5F4FF] active:scale-[0.98]"><Printer className="h-4 w-4" />PDF 報告</button><button type="button" onClick={onExportExcel} className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/40 bg-white/10 px-4 text-[13px] font-bold text-white transition hover:bg-white/20 active:scale-[0.98]"><FileSpreadsheet className="h-4 w-4" />Excel 報告</button></div></div></section>
      ...
      <section className="mt-7 rounded-2xl border border-[#E0E3EF] bg-[#FCFCFF] p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-[12px] font-bold tracking-[0.11em] text-[#837CA9]">訓練目標</p><h2 className="mt-1 text-[20px] font-bold text-[#30395F]">已選訓練目標</h2></div><span className="rounded-lg bg-[#E9E7FF] px-3 py-2 text-[12px] font-bold text-[#5C50D5]">{selectedGoals.length} 項</span></div>{selectedGoalItems.length ? <div className="mt-4 overflow-x-auto rounded-xl border border-[#E1E4EF] bg-white"><table className="w-full min-w-[860px] table-fixed border-collapse text-left"><thead className="bg-[#F6F6FD]"><tr><th className="w-[155px] border-b border-r border-[#E1E4EF] px-3 py-3 text-[12px] font-bold text-[#59638E]">大類別</th><th className="w-[220px] border-b border-r border-[#E1E4EF] px-3 py-3 text-[12px] font-bold text-[#59638E]">小範疇</th>{childNames.map((name, childIndex) => <th key={childIndex} className="border-b border-r border-[#E1E4EF] px-3 py-3 text-center text-[12px] font-bold text-[#59638E] last:border-r-0">{displayChildName(name, childIndex)}</th>)}</tr></thead><tbody>{selectedGoalItems.map((item) => <tr key={item.id} className="border-b border-[#EDF0F6] last:border-b-0"><td className="border-r border-[#E7E9F1] px-3 py-3 text-[12px] font-bold text-[#59638E]">{cleanHeading(item.context)}</td><th scope="row" className="border-r border-[#E7E9F1] px-3 py-3 text-left text-[13px] leading-5 font-semibold text-[#384263]"><ItemText text={item.text} /></th>{childNames.map((_, childIndex) => { const selected = selectedGoals.includes(goalKey(item.id, childIndex)); return <td key={childIndex} className="border-r border-[#E7E9F1] p-2 text-center last:border-r-0"><button type="button" onClick={() => selected && onToggleGoal(item.id, childIndex)} disabled={!selected} aria-label={selected ? `移除 ${displayChildName(childNames[childIndex], childIndex)} 的訓練目標` : undefined} className={`inline-flex h-7 min-w-7 items-center justify-center rounded-md border text-[12px] font-bold ${selected ? "border-[#6D5DFB] bg-[#6D5DFB] px-2 text-white" : "border-transparent bg-transparent text-[#C4C9D7]"}`}>{selected ? <><Check className="mr-1 h-3.5 w-3.5" />目標</> : "—"}</button></td>; })}</tr>)}</tbody></table></div> : <p className="mt-3 text-[13px] text-[#838AAA]">按展開格子中的方框，即可加入個別兒童的訓練目標。</p>}</section>
    </main>
  </div>;
}
