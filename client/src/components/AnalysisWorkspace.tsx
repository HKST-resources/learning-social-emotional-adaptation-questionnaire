...
function OverviewTable({ category, areas, childNames, ratings, scoreFilter, selectedGoals, onToggleGoal }: {
  ...
  return <section className="mt-7">
    <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
      ...
      <p className="text-[12px] text-[#757FA6]">格內數字為小範疇平均分數；每位兒童最低的五個平均分數以漸變色標示。</p>
    </div>
    <div className="comparison-scroll overflow-x-auto rounded-2xl border border-[#E1E4EF] bg-white shadow-[0_8px_24px_rgba(42,45,88,0.06)]">
      <table className="w-full min-w-[370px] table-fixed border-collapse text-left sm:min-w-0">
        <thead className="bg-[#F7F8FE]"><tr>
          <th className="w-[54px] border-b border-r border-[#E1E4EF] px-1.5 py-3 text-[10px] font-bold text-[#57618B] sm:w-[100px] sm:px-2 sm:text-[12px]">大類別</th>
          <th className="w-[102px] border-b border-r border-[#E1E4EF] px-1.5 py-3 text-[10px] font-bold text-[#57618B] sm:w-[170px] sm:px-2 sm:text-[12px]">小範疇</th>
          {childNames.map((name, index) => <th key={index} className="border-b border-r border-[#E1E4EF] px-0.5 py-3 text-center last:border-r-0 sm:px-2"><p className="text-[9px] font-bold text-[#8A91B0]">兒童 {index + 1}</p><p className="mt-0.5 truncate text-[11px] font-extrabold text-[#444F7A] sm:text-[13px]">{displayChildName(name, index)}</p></th>)}
        </tr></thead>
        <tbody>{areas.map((area) => <Fragment key={area.key}>
          <tr id={`analysis-area-${area.key}`} className="border-b border-[#E7E9F1] align-stretch">
            <td className="border-r border-[#E7E9F1] bg-[#FBFAFF] px-1.5 py-3 text-[10px] font-bold leading-4 text-[#59638E] sm:px-2 sm:text-[12px]"><span className="line-clamp-3">{cleanHeading(area.context)}</span></td>
            <th scope="row" className="border-r border-[#E7E9F1] px-1.5 py-3 text-left text-[11px] font-bold leading-4 text-[#30395F] sm:px-2 sm:text-[13px]"><span className="line-clamp-3">{cleanHeading(area.group)}</span></th>
            {childNames.map((_, childIndex) => { const score = area.scores[childIndex]; return <td key={childIndex} className="border-r border-[#E7E9F1] p-1 text-center last:border-r-0 sm:p-1.5"><div className={`flex min-h-10 w-full items-center justify-center rounded-lg border px-0.5 text-[12px] font-extrabold sm:text-[14px] ${meanTone(lowestMeanAreaKeys[childIndex].has(area.key), score.average)}`}>{score.average === undefined ? "—" : score.average.toFixed(2)}</div></td>; })}
          </tr>
          {area.items.map((item, itemIndex) => <tr id={`analysis-item-${item.id}`} key={item.id} className="border-b border-[#EEF0F6] bg-[#FCFCFF] last:border-b-0">
            <td colSpan={2} className="border-r border-[#E7E9F1] px-1.5 py-2 align-top sm:px-2"><div className="flex gap-1.5"><span className="mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-[#F0EFFF] text-[9px] font-bold text-[#6557DB] sm:h-5 sm:w-5 sm:text-[10px]">{itemIndex + 1}</span><p className="text-[11px] leading-4 font-semibold text-[#394264] sm:text-[13px] sm:leading-5"><ItemText text={item.text} /></p></div></td>
            {childNames.map((name, childIndex) => { const entry = ratings[goalKey(item.id, childIndex)]; const visible = entry?.score !== undefined && scoreFilter.has(entry.score); const checked = selectedGoals.includes(goalKey(item.id, childIndex)); return <td key={childIndex} className="border-r border-[#E7E9F1] px-0.5 py-1 align-top last:border-r-0 sm:px-1 sm:py-1.5"><div className={visible ? "min-h-[38px] rounded-md border border-[#E2E5EF] bg-white p-1 sm:min-h-[42px] sm:p-1.5" : "invisible min-h-[38px] rounded-md border border-[#E2E5EF] bg-white p-1 sm:min-h-[42px] sm:p-1.5"}><div className="flex gap-1"><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-md border text-[10px] font-extrabold sm:h-6 sm:w-6 sm:text-[12px] ${entry?.score === undefined ? "border-[#E5E7EF] text-[#ADB3C6]" : quietRatingClass[entry.score]}`}>{entry?.score ?? "—"}</span><button type="button" onClick={() => onToggleGoal(item.id, childIndex)} aria-pressed={checked} aria-label={`${checked ? "移除" : "選擇"}${displayChildName(name, childIndex)} 的訓練目標：${item.text.split("\n")[0]}`} className={`ml-auto flex h-5 w-5 shrink-0 items-center justify-center rounded-md border transition sm:h-6 sm:w-6 ${checked ? "border-[#2AA876] bg-[#2AA876] text-white" : "border-[#CBD0E1] bg-white text-transparent hover:border-[#2AA876]"}`}><Check className="h-3 w-3 sm:h-3.5 sm:w-3.5" /></button></div>{entry?.remark ? <p className="mt-1 border-t border-[#EEF0F6] pt-1 text-[9px] leading-3.5 text-[#737CA1] sm:text-[10px] sm:leading-4">備註：{entry.remark}</p> : null}</div></td>; })}
          </tr>)}
        </Fragment>)}</tbody>
      </table>
    </div>
