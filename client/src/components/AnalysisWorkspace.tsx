...
function DetailScore({ item, childIndex, childName, entry, selected, visible, onToggleGoal }: {
  ...
  return <button type="button" onClick={() => onToggleGoal(item.id, childIndex)} aria-pressed={selected} aria-label={`${selected ? "取消" : "選擇"}${displayChildName(childName, childIndex)} 的訓練目標：${item.text.split("\n")[0]}`} className={`grid h-8 w-full grid-cols-[1fr_auto_1fr] items-center rounded-full border px-2 text-[12px] font-extrabold transition active:scale-[0.97] sm:h-9 sm:text-[13px] ${selected ? "border-[3px] border-[#15975C] bg-[#DDF8E9] text-[#087044] shadow-[0_0_0_2px_rgba(21,151,92,0.15)]" : scoreTone[entry.score]}`}><span className="col-start-2">{entry.score}</span>{selected && <Check className="col-start-3 justify-self-end h-4 w-4 stroke-[3] sm:h-5 sm:w-5" />}</button>;
}
...
function OverviewTable({ category, sectionId, areas, childNames, ratings, scoreFilter, selectedGoals, onToggleGoal, priorityKeys, expandedKeys, onToggleArea }: {
  ...
  return <section id={sectionId} className="scroll-mt-5 mt-7">
    ...
    <div className="comparison-scroll overflow-x-auto rounded-2xl border border-[#E1E4EF] bg-white shadow-[0_8px_24px_rgba(42,45,88,0.06)]"><table className="w-full min-w-[356px] table-fixed border-collapse text-left sm:min-w-0"><thead className="bg-[#F7F8FE]"><tr><th className="w-[48px] border-b border-r border-[#E1E4EF] px-1 py-3 text-[9px] font-bold text-[#57618B] sm:w-[80px] sm:px-2 sm:text-[12px]">類別</th><th className="w-[96px] border-b border-r border-[#E1E4EF] px-1 py-3 text-[9px] font-bold text-[#57618B] sm:w-[144px] sm:px-2 sm:text-[12px]">小範疇</th>{childNames.map((name, index) => <th key={index} className="w-[52px] border-b border-r border-[#E1E4EF] px-0.5 py-3 text-center last:border-r-0 sm:w-[78px] sm:px-1"><p className="text-[8px] font-bold text-[#8A91B0] sm:text-[9px]">兒童 {index + 1}</p><p className="mt-0.5 truncate text-[10px] font-extrabold text-[#444F7A] sm:text-[12px]">{displayChildName(name, index)}</p></th>)}</tr></thead><tbody>{areas.map((area, index) => {
