 ...
 * This is a comparison ledger, not a card stack. Expanded rating items and the score capsules
 * occupy matching table rows; rounded item scores are visibly distinct from mean-score grids.
 */
...
function sectionKey(sectionId: string, context: string) { return `${sectionId}-${context.replace(/[^\w\u4e00-\u9fff]/g, "-")}`; }
...
function DetailScore({ item, childIndex, childName, entry, selected, visible, onToggleGoal }: {
  ...
  return <button type="button" onClick={() => onToggleGoal(item.id, childIndex)} aria-pressed={selected} aria-label={`${selected ? "取消" : "選擇"}${displayChildName(childName, childIndex)} 的訓練目標：${item.text.split("\n")[0]}`} className={`grid h-8 w-full grid-cols-[1fr_auto_1fr] items-center rounded-full border px-2 text-[12px] font-extrabold transition active:scale-[0.97] sm:h-9 sm:text-[13px] ${scoreTone[entry.score]} ${selected ? "border-[3px] border-[#08B85C] shadow-[0_0_0_3px_rgba(0,184,92,0.36),0_0_16px_rgba(0,184,92,0.52)" : ""}`}><span className="col-start-2">{entry.score}</span>{selected && <Check className="col-start-3 justify-self-end h-4 w-4 stroke-[3] text-[#08B85C] drop-shadow-[0_1px_0_rgba(255,255,255,0.95)] sm:h-5 sm:w-5" />}</button>;
}
...
function OverviewTable({ category, sectionId, areas, childNames, ratings, scoreFilter, selectedGoals, onToggleGoal, priorityKeys, expandedKeys, onToggleArea }: {
  ...
  const contextRowSpans = useMemo(() => {
    ...
    areas.forEach((area, index) => {
      ...
      let rows = 0;
      let cursor = index;
      while (areas[cursor] && areas[cursor].context === area.context) { rows += 1 + (expandedKeys.has(areas[cursor].key) ? areas[cursor].items.length : 0); cursor += 1; }
      result.set(index, rows);
    });
    return result;
  }, [areas, expandedKeys]);
  const contextButtons = useMemo(() => areas.filter((area, index) => !index || areas[index - 1].context !== area.context).map((area) => ({ context: area.context, active: areas.filter((candidate) => candidate.context === area.context).some((candidate) => priorityKeys.some((keys) => keys.has(candidate.key))) })), [areas, priorityKeys]);
  return <section id={sectionId} className="scroll-mt-5 mt-7">
    <div className="mb-3 flex flex-wrap items-center gap-2"><p className="text-[12px] font-bold tracking-[0.11em] text-[#837CA9]">總覽</p><h2 className="text-[22px] font-bold text-[#30395F]">{category}</h2>{contextButtons.map(({ context, active }) => <button key={context} type="button" onClick={() => document.getElementById(sectionKey(sectionId, context))?.scrollIntoView({ behavior: "smooth", block: "start" })} className={`rounded-lg border px-3 py-1.5 text-[12px] font-bold transition active:scale-[0.98] ${active ? "border-[#8B69ED] bg-[linear-gradient(100deg,#E9E4FF,#FCE4F2)] text-[#5646C5] shadow-[0_3px_8px_rgba(113,91,231,0.14)]" : "border-[#DDE0EC] bg-white text-[#667097] hover:bg-[#F7F7FC]"}`}>{context}</button>)}</div>
    <p className="mb-3 text-[12px] text-[#757FA6]">格內數字為小範疇平均分數；按分數格可展開詳細評分。第五個最低分數如有同分，一併以漸變色標示。</p>
    <div className="comparison-scroll overflow-x-auto rounded-2xl border border-[#E1E4EF] bg-white shadow-[0_8px_24px_rgba(42,45,88,0.06)]"><table className="w-full min-w-[356px] table-fixed border-collapse text-left sm:min-w-0"><thead className="bg-[#F7F8FE]"><tr><th className="w-[48px] border-b border-r border-[#E1E4EF] px-1 py-3 text-[9px] font-bold text-[#57618B] sm:w-[80px] sm:px-2 sm:text-[12px] md:w-[74px]">類別</th><th className="w-[96px] border-b border-r border-[#E1E4EF] px-1 py-3 text-[9px] font-bold text-[#57618B] sm:w-[144px] sm:px-2 sm:text-[12px] md:w-[220px]">小範疇</th>{childNames.map((name, index) => <th key={index} className="w-[52px] border-b border-r border-[#E1E4EF] px-0.5 py-3 text-center last:border-r-0 sm:w-[78px] sm:px-1 md:w-auto"><p className="text-[8px] font-bold text-[#8A91B0] sm:text-[9px]">兒童 {index + 1}</p><p className="mt-0.5 truncate text-[10px] font-extrabold text-[#444F7A] sm:text-[12px]">{displayChildName(name, index)}</p></th>)}</tr></thead><tbody>{areas.map((area, index) => {
      ...
      const contextStart = contextSpan !== undefined;
      return <FragmentArea key={area.key} area={area} index={index} expanded={expanded} surface={surface} contextSpan={contextSpan} contextStart={contextStart} contextId={sectionKey(sectionId, area.context)} childNames={childNames} ratings={ratings} scoreFilter={scoreFilter} selectedGoals={selectedGoals} onToggleGoal={onToggleGoal} priorityKeys={priorityKeys} onToggleArea={onToggleArea} />;
    })}</tbody></table></div>
  </section>;
}

function FragmentArea({ area, index, expanded, surface, contextSpan, contextStart, contextId, childNames, ratings, scoreFilter, selectedGoals, onToggleGoal, priorityKeys, onToggleArea }: {
  area: SummaryArea;
  index: number;
  expanded: boolean;
  surface: string;
  ...
  return <>
    ...
    {expanded && area.items.map((item, itemIndex) => <tr id={`analysis-item-${item.id}`} key={item.id} className={`border-b border-[#E7E9F1] ${surface}`}><td className="border-r border-[#E7E9F1] px-1 py-1.5 sm:px-2"><div className="flex min-h-8 items-center gap-1.5 rounded-lg border border-[#DED9FF] bg-[#F1F0FF] px-1.5 py-1.5 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.7)] sm:min-h-9 sm:px-2"><span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-[#7465E9] text-[9px] font-bold text-white">{itemIndex + 1}</span><p className="text-[10px] font-semibold leading-4 text-[#433D83] sm:text-[12px]"><ItemText text={item.text} /></p></div></td>{childNames.map((name, childIndex) => { const entry = ratings[goalKey(item.id, childIndex)]; const selected = selectedGoals.includes(goalKey(item.id, childIndex)); const visible = entry?.score !== undefined && scoreFilter.has(entry.score); return <td key={childIndex} className="border-r border-[#E7E9F1] p-1 align-middle text-center last:border-r-0"><DetailScore item={item} childIndex={childIndex} childName={name} entry={entry} selected={selected} visible={visible} onToggleGoal={onToggleGoal} /></td>; })}</tr>)}
  </>;
}
...
export default function AnalysisWorkspace({ ratingDate, childNames, items, ratings, selectedGoals, onToggleGoal, onClearGoals, onBack, onExportExcel }: Props) {
  ...
  const priorityKeys = useMemo(() => childNames.map((_, childIndex) => { const ranked = areas.filter((area) => area.scores[childIndex].average !== undefined).sort((left, right) => (left.scores[childIndex].average ?? Infinity) - (right.scores[childIndex].average ?? Infinity)); const threshold = ranked[Math.min(4, ranked.length - 1)]?.scores[childIndex].average; return new Set(ranked.filter((area) => threshold !== undefined && (area.scores[childIndex].average ?? Infinity) <= threshold).map((area) => area.key)); }), [areas, childNames]);
